...
const typeColor = (type: string) => {
  ...
  if (t === 'dashboard') return 'text-[var(--accent-cyan)] bg-[var(--accent-cyan)]/8 border-[var(--accent-cyan)]/20';
  return 'text-[var(--text-tertiary)] bg-white/[0.03] border-white/[0.06]';
};

export default function BlogPostLayout({
  children, title, date, category, type, excerpt, readingTime, sourceUrl, sourceLabel, author,
}: BlogPostLayoutProps) {
  return (
      ...
      <div className="max-w-[720px] mx-auto px-6 md:px-8 pt-20 pb-24">
        ...
        <div className="mb-10">
          <div className="flex flex-wrap items-center gap-2 mb-4">
            <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold tracking-wide uppercase border ${categoryColor(category)}`}>{category}</span>
            <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold tracking-wide uppercase border ${typeColor(type)}`}>{type}</span>
          </div>
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-semibold tracking-[-1.5px] leading-tight mb-6">{title}</h1>
          <div className="flex items-center gap-4 text-sm text-[var(--text-muted)]">
            <span>{date}</span>
            {readingTime && <><span className="text-[var(--text-disabled)]">·</span><span>{readingTime}</span></>}
            {author && <><span className="text-[var(--text-disabled)]">·</span><span>{author}</span></>}
            {sourceUrl && <><span className="text-[var(--text-disabled)]">·</span><a href={sourceUrl} target="_blank" rel="noopener" className="text-[var(--accent-cyan)] hover:underline">{sourceLabel || 'Source'}</a></>}
          </div>
        </div>
        ...
        <div className="mt-16 pt-8 border-t border-[var(--border-default)]">
          <div className="flex items-center justify-between">
            ...
            <Link href="/blog/" className="inline-flex items-center gap-1.5 text-xs text-[var(--accent-cyan)] hover:underline">
              More articles
