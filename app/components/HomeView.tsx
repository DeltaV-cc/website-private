...
export default function HomeView({ lang }: { lang: Locale }) {
  ...
  return <main className="relative isolate">
    ...
    <section id="offerings" className="border-y border-[var(--border-default)] bg-[var(--bg-surface)]" aria-labelledby="offerings-heading"><div className="page-container py-20 md:py-28"><div className="text-center"><div className="eyebrow text-[var(--accent-orange)]">{copy.offeringsEyebrow}</div><h2 id="offerings-heading" className="section-title mx-auto mt-3 max-w-3xl text-center text-4xl md:text-6xl lg:text-7xl">{copy.offeringsTitle}</h2></div><div className="mt-14 divide-y divide-[var(--border-default)] border-y border-[var(--border-default)]">{OFFERING_STRUCTURE.map((s, i) => { const o = copy.offerings[i]; return <Link key={s.index} href={to(s.href)} className="forge-course-link group grid md:grid-cols-[5rem_minmax(0,1fr)_12rem] gap-5 items-start py-7"><span className="font-mono text-xs text-[var(--accent-cyan)]">{s.index}</span><span><h3 className="text-xl font-semibold">{o.title}</h3><p className="mt-2 max-w-xl text-sm text-[var(--text-secondary)]">{o.text}</p></span><span className={`text-sm justify-self-start ${s.ctaColor}`}>{o.cta} <Arrow /></span></Link>; })}</div></div></section>

    <section className="border-y border-[var(--border-default)] bg-[var(--bg-surface)] pb-24" aria-labelledby="cta-heading"><div className="page-container pt-8 md:pt-14"><div className="relative w-full overflow-hidden"><div className="relative z-10"><div className="relative overflow-hidden border border-[var(--border-default)] bg-[var(--bg-deep)]"><div className="relative h-48 md:h-56 overflow-hidden"><div className="absolute left-8 top-6 z-10 md:left-10"><div className="eyebrow text-[var(--accent-cyan)]">{copy.loopEyebrow}</div></div><div className="absolute inset-0"><InkGarden compact /></div><div className="absolute inset-0 bg-gradient-to-r from-[var(--bg-deep)]/90 via-[var(--bg-deep)]/55 to-transparent" /><h2 id="cta-heading" className="section-title absolute inset-x-0 top-[42%] max-w-xl -translate-y-1/2 p-8 md:p-10">{copy.loopTitle}</h2><p className="absolute bottom-0 left-0 z-10 max-w-xl p-8 pb-5 md:p-10 md:pb-7 text-[var(--text-secondary)]">{copy.loopBlurb}</p></div></div><div className="mt-10"><CuratedIntel /></div><div className="mt-6 flex flex-wrap items-center justify-between gap-4"><div className="flex flex-wrap gap-3"><Link href="/tutorials/" className="button-secondary">{copy.tutorials} <Arrow /></Link><Link href="/blog/" className="button-secondary">{copy.blog} <Arrow /></Link></div><Link href={to('/contact/')} className="button-secondary">{copy.getInTouch} <Arrow /></Link></div></div></div></div></section>
    </div>
  </main>;
}
