...
type SampledCell = { x: number; y: number; ix: number; iy: number; level: number; fill: string; mode: RenderMode };
...
export default function InkGarden({ compact = false, background = false, source = sourcePhoto, renderMode = 'mixed', className = '' }: InkGardenProps) {
  ...
  useEffect(() => {
    ...
    const photo = new Image(); photo.decoding = 'async'; photo.loading = 'lazy'; photo.fetchPriority = background ? 'low' : 'auto';
    const sourceCanvas = document.createElement('canvas'); const sourceCtx = sourceCanvas.getContext('2d');
    const blurredCanvas = document.createElement('canvas'); const blurredCtx = blurredCanvas.getContext('2d');
    let raf = 0; let resizeRaf = 0; let lastFrame = 0; let imageData: Uint8ClampedArray | null = null; let sampledCells: SampledCell[] = []; let prepared = false; let visible = background; let loadStarted = false;
    ...
    const resize = () => { if (resizeRaf) return; resizeRaf = requestAnimationFrame(() => { resizeRaf = 0; const ratio = background ? 1 : Math.min(window.devicePixelRatio || 1, 2); const { w, h } = renderSize(); canvas.width = w * ratio; canvas.height = h * ratio; ctx.setTransform(ratio, 0, 0, ratio, 0, 0); sourceCanvas.width = w; sourceCanvas.height = h; blurredCanvas.width = w; blurredCanvas.height = h; imageData = null; sampledCells = []; prepared = false; if (photo.complete && photo.naturalWidth && sourceCtx) prepare(); else draw(performance.now()); }); };
    const prepare = () => { if (!photo.complete || !photo.naturalWidth || !sourceCtx || !blurredCtx) return; const { w, h } = renderSize(); const scale = Math.max(w / photo.naturalWidth, h / photo.naturalHeight); const sw = photo.naturalWidth * scale; const sh = photo.naturalHeight * scale; const imagePosition = background ? (window.innerWidth <= 640 ? .24 : .52) : .38; const imageX = (w - sw) / 2 + Math.max(0, sw - w) * imagePosition; const imageY = (h - sh) / 2; cometHead = { x: (imageX + sourceComet.headX * scale) / w, y: (imageY + sourceComet.headY * scale) / h }; cometTail = { x: (imageX + sourceComet.tailX * scale) / w, y: (imageY + sourceComet.tailY * scale) / h }; sourceCtx.clearRect(0, 0, w, h); sourceCtx.globalAlpha = settings.bgOpacity / 100; sourceCtx.drawImage(photo, imageX, imageY, sw, sh); sourceCtx.globalAlpha = 1; imageData = sourceCtx.getImageData(0, 0, w, h).data;
      blurredCtx.clearRect(0, 0, w, h); blurredCtx.save(); blurredCtx.filter = 'blur(.8px) brightness(1.08) saturate(1.08)'; blurredCtx.drawImage(sourceCanvas, 0, 0, w, h); blurredCtx.restore();
      const cols = Math.ceil(w / settings.cellSize); const rows = Math.ceil(h / (settings.cellSize * 1.35)); sampledCells = new Array(cols * rows); let cellIndex = 0;
      for (let iy = 0; iy < rows; iy++) for (let ix = 0; ix < cols; ix++) { const px = Math.min(w - 1, Math.floor((ix + .5) * settings.cellSize)); const py = Math.min(h - 1, Math.floor((iy + .5) * settings.cellSize * 1.35)); const index = (py * w + px) * 4; const r = imageData[index] ?? 75; const g = imageData[index + 1] ?? 62; const b = imageData[index + 2] ?? 80; const level = (r * .299 + g * .587 + b * .114) / 255; let mode: RenderMode = renderMode; if (renderMode === 'mixed') { const pattern = (ix * 7 + iy * 11) % 12; mode = pattern < 5 ? 'characters' : pattern < 8 ? 'pixel' : pattern < 10 ? 'lines' : 'halfblocks'; } sampledCells[cellIndex++] = { x: px, y: py, ix, iy, level, fill: colour(r, g, b, settings), mode }; }
      prepared = true; imageData = null;
    };
    ...
    const draw = (time: number) => {
      ...
      if (prepared && sourceCanvas.width === w) { ctx.save(); ctx.globalAlpha = background ? .66 : .46; ctx.drawImage(blurredCanvas, 0, 0, w, h); ctx.restore(); }
      ...
      const cols = Math.ceil(w / settings.cellSize); const rows = Math.ceil(h / (settings.cellSize * 1.35));
      ctx.save(); ctx.globalAlpha = background ? .3 : .34;
      for (let cellIndex = 0; cellIndex < sampledCells.length; cellIndex++) { const cell = sampledCells[cellIndex]; const ix = cell.ix; const iy = cell.iy; const px = cell.x; const py = cell.y; let level = cell.level;
        ...
        const glow = Math.max(tailGlow, headGlow); const redAmount = 1 - projection; const cometFill = tailGlow > .06 ? `rgb(${Math.round(70 + redAmount * 180)},${Math.round(155 - redAmount * 80)},${Math.round(205 - redAmount * 90)})` : headGlow > .08 ? `rgb(${Math.round(210 + headGlow * 45)},${Math.round(175 + headGlow * 65)},${Math.round(160 + headGlow * 75)})` : null; const fill = cometFill || (glow > .08 ? `rgb(${Math.round(100 + glow * 155)},${Math.round(190 + glow * 65)},255)` : cell.fill); const artFlicker = reduce ? 0 : Math.sin(time * .004 * settings.animSpeed + ix * .23 + iy * .17) * .5 + .5; ctx.globalAlpha = background ? .22 + artFlicker * .1 : .34; const wind = background || reduce ? 0 : Math.sin(time * .0014 * settings.animSpeed + iy * .15 + ix * .018) * Math.max(0, (yNorm - .68) / .32) * settings.cellSize * .62; const shapeSize = background ? settings.cellSize * .82 : settings.cellSize; drawShape(cell.mode, px + wind, py, shapeSize, level, fill, time, ix, iy);
      }
      ctx.restore();
    ...
    window.addEventListener('resize', resize); document.addEventListener('visibilitychange', onVisibility); return () => { cancelAnimationFrame(raf); cancelAnimationFrame(resizeRaf); observer?.disconnect(); window.removeEventListener('resize', resize); document.removeEventListener('visibilitychange', onVisibility); };
