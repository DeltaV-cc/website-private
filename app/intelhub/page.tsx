...
/* ================================================================
   DASHBOARDS — restructured per Marc's vision
   ================================================================ */
...
const DASHES: Record<string, DashDef> = {
  macro: {
    ...
    sources: [
      ...
      {name:'Brian Roemmele',color:'border-l-sky-400',match:['brianroemmele']},
      {name:'Teknium',color:'border-l-indigo-400',match:['teknium']},
      {name:'DeepSeek',color:'border-l-blue-400',match:['deepseek']},
      {name:'Qwen',color:'border-l-cyan-400',match:['alibaba_qwen','qwen']},
  ...
  infosec: {
    accent: 'text-red-400', title: 'Infosec — Live CVEs · Crypto Hacks · Protocol Upgrades',
    tags: ['cybersec','opsec'],
    chart: false,
    sources: [
      ...
      {name:'Hacker News',color:'border-l-yellow-400',match:['hacker news']},
  ...
  web3: {
    accent: 'text-purple-400', title: 'Web3 — TVL · Stablecoins · DEX Volume · DeFi',
    tags: ['crypto','web3'],
    chart: true, // <-- show DeFiLlama chart data
    sources: [
...
interface ChartData{totalTVL:number;dexVolume24h:number;fees24h:number;topProtocols:any[];chainData:any[];stablecoinData:any[];}
...
export default function IntelHubPage(){
  ...
  // Fetch charts only for Web3 dashboard
  ...
  const dashItems=items.filter(i=>{const src=dd.sources.some(s=>s.match.some(k=>i.source?.toLowerCase().includes(k)));const tag=dd.tags.some(t=>i.tag===t);return src||tag;}).slice(0,15);
          ...
          <div className="mb-8 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.015] p-4">
                <div className="text-[10px] text-white/20 uppercase tracking-wider">Total TVL</div>
                <div className="text-2xl font-bold tabular-nums mt-1 text-emerald-400">{fmt(charts.totalTVL)}</div>
              </div>
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.015] p-4">
                <div className="text-[10px] text-white/20 uppercase tracking-wider">DEX Volume (24h)</div>
                <div className="text-2xl font-bold tabular-nums mt-1 text-purple-400">{fmt(charts.dexVolume24h)}</div>
              </div>
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.015] p-4">
                <div className="text-[10px] text-white/20 uppercase tracking-wider">Fees (24h)</div>
                <div className="text-2xl font-bold tabular-nums mt-1 text-amber-400">{fmt(charts.fees24h)}</div>
              </div>
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.015] p-4">
                <div className="text-[10px] text-white/20 uppercase tracking-wider">Updated</div>
                <div className="text-base font-semibold mt-1 text-white/50">{ago(charts.updated)} ago</div>
              </div>
            </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {charts.topProtocols.slice(0,8).map((p:any,i:number)=>(
                    <div key={i} className="rounded-xl border border-white/[0.05] bg-white/[0.01] p-3 hover:border-white/15 transition-all">
                      <div className="flex items-center justify-between"><span className="text-[12px] font-medium text-white/70 truncate">{p.name}</span><span className={`text-[10px] ${p.change_1d>=0?'text-emerald-400':'text-red-400'}`}>{p.change_1d>=0?'+':''}{p.change_1d?.toFixed(1)}%</span></div>
              ...
              <div>
                ...
                <div className="flex gap-3 overflow-x-auto no-scrollbar">
                  {charts.stablecoinData.map((s:any,i:number)=>(
                    <div key={i} className="flex-shrink-0 w-[160px] rounded-xl border border-white/[0.05] bg-white/[0.01] p-3">
                      <div className="text-[12px] font-medium text-white/70">{s.name}</div>
                      <div className="text-[11px] text-white/40">{s.symbol}</div>
                      <div className="text-[15px] font-bold mt-1 text-white/80">{fmt(s.circulating)}</div>
                    </div>
                  ))}
                </div>
              </div>
