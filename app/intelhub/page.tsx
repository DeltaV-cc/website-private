...
/* ================================================================
   TIGHTER KEYWORD MATCHING — avoid greedy hits like "model" → #ai
   ================================================================ */
const KEYWORDS: Record<string, string> = {
  // ordered: first match wins — put more specific / rarer terms first
  cybersec: 'cybersec,cve,vulnerability,breach,ransomware,outage,bgp,ddos,cisa,kev,hibp',
  opsec: 'opsec,hardening,threat model,zero-day,privacy,air-gapped',
  macro: 'macro,geopolitic,sanction,ofac,tariff,export control,trade war,federal reserve,bis,un security,energy',
  crypto: 'bitcoin,ethereum,token,defi,yield,liquidity pool,dex,amm,polymarket,kalshi,orderbook,perp',
  web3: 'web3,blockchain,evm,on-chain,dao,nft,smart contract,solidity,rollup,l2',
  ai: 'llm,transformer,gpt,claude,deepseek,mistral,gemma,local-first,sovereign ai,lora,qlora',
  hardware: 'chip,gpu,cpu,npu,meshtastic,iot,compute cluster,h100,a100,b200',
  prediction: 'prediction market,forecast,odds,event contract',
  ip: 'patent,uspto,wipo,trademark,copyright,intellectual property',
  legal: 'regulation,law,compliance,sec,ftc,cfpb,doj,antitrust,gdpr',
...
const DASH: Record<string, { accent: string; title: string; sources: string[] }> = {
  macro: {
    ...
    sources: ['OFAC', 'BIS', 'ITA', 'UN', 'Sanctions', 'Trade', 'Tariff', 'Export', 'Federal'],
  },
  infosec: {
    accent: 'text-orange-400',
    title: 'Cybersecurity & Infrastructure',
    sources: ['CISA', 'ICS-CERT', 'NIST', 'HIBP', 'IODA', 'BGP', 'Vulnerability', 'CVE', 'Breach', 'Outage', 'Malware'],
  },
  web3: {
    accent: 'text-purple-400',
    title: 'Crypto & DeFi',
    sources: ['DeFiLlama', 'Polymarket', 'IPFS', 'Protocol Labs', 'Filecoin', 'Yield', 'Token', 'Volume', 'DEX'],
  },
};

/* ================================================================
   POLYMARKET FILTER — only keep relevant markets
   ================================================================ */
const POLY_TOPICS = ['crypto', 'bitcoin', 'ethereum', 'macro', 'election', 'fed', 'tariff', 'ai', 'gdp', 'inflation', 'rate', 'treasury', 'sec', 'regulation', 'war'];

function isRelevantPoly(item: { title: string; source: string }) {
  if (!item.source?.toLowerCase().includes('polymarket')) return true; // not poly → keep
  const t = item.title.toLowerCase();
  return POLY_TOPICS.some(k => t.includes(k));
}
  ...
  const load = async () => {
    ...
    try {
      ...
      const enriched = d
        ...
        .filter(isRelevantPoly); // drop irrelevant Polymarket items
      setItems(enriched);
    } catch (e) {
      console.error(e);
    } finally {
    ...
    <div className="min-h-screen bg-[#040407] text-white">
              ...
              >
                <div className="flex items-start gap-2">
                  <div className="flex-1 min-w-0">
                    <div className="text-[13px] font-medium leading-snug line-clamp-2 text-white/85 group-hover:text-white transition-colors">{it.title}</div>
                  </div>
                ...
                <div className="flex items-center gap-2 mt-3 text-[11px] text-white/25">
                  {it.tag && (
                    <span className={`px-2 py-0.5 rounded-md font-semibold text-[10px] tracking-wide ${TC[it.tag]?.split(' ')[0] || ''} ${TC[it.tag]?.split(' ')[1] || ''}`}>
                      #{it.tag}
                    </span>
                  )}
          ...
          {top3.map((it, i) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 mt-5 text-[12px] text-white/25">
                {it.tag && (
                  <span className={`px-2.5 py-1 rounded-lg font-semibold text-[11px] ${TC[it.tag]?.split(' ')[0] || ''} ${TC[it.tag]?.split(' ')[1] || ''}`}>#{it.tag}</span>
                )}
                <span>{it.source}</span>
      ...
      <div className="max-w-[1440px] mx-auto px-8 pb-24">
        ...
        <div className="rounded-2xl border border-white/[0.06] overflow-hidden bg-white/[0.01]">
          ...
          {di.length === 0 && <div className="px-5 py-12 text-center text-white/15 text-sm italic">No high‑signal items yet — triage is processing.</div>}
          {di.map((it, i) => (
            <a
              key={i}
              href={it.url}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {di.slice(0, 6).map((it, i) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 mt-3 text-[11px] text-white/25">
                  {it.tag && (
                    <span className={`px-2 py-0.5 rounded-md font-semibold text-[10px] ${TC[it.tag]?.split(' ')[0] || ''} ${TC[it.tag]?.split(' ')[1] || ''}`}>#{it.tag}</span>
                  )}
                  <span>{it.source}</span>
