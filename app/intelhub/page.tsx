...
export default function IntelHubPage(){
  ...
  function TileRow({it}:{it:Item}){return<a href={it.url} target="_blank" rel="noopener noreferrer" className="block px-4 py-2.5 hover:bg-white/[0.03] group"><div className="text-[11px] font-medium text-white/60 group-hover:text-white/85 line-clamp-2 leading-snug">{it.title}</div><div className="text-[9px] text-white/20 mt-1">{ago(it.published_at)}</div></a>;}
      ...
      <div className="max-w-[1440px] mx-auto px-8 pb-24">
        ...
        {active==='macro'&&(<div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="rounded-2xl border border-white/[0.06] bg-white/[0.01] p-4 flex flex-col items-center"><div className="text-[9px] text-amber-400 uppercase tracking-[.15em] font-bold mb-2">F&G</div><div className="relative h-24 w-5 bg-white/[0.04] rounded-full overflow-hidden mb-1"><div className={`absolute bottom-0 w-full rounded-full ${fgVal>50?'bg-emerald-500/60':fgVal<30?'bg-red-500/60':'bg-amber-500/60'}`} style={{height:`${Math.max(3,fgVal)}%`}}/></div><div className={`text-lg font-bold ${fgVal>50?'text-emerald-400':fgVal<30?'text-red-400':'text-amber-400'}`}>{fgVal||'--'}</div><div className="text-[9px] text-white/25">{fgLabel||'...'}</div></div>
            {/* Forex as compact chart */}
            <div className="rounded-2xl border border-white/[0.06] bg-white/[0.01] p-4"><div className="text-[9px] text-sky-400 uppercase tracking-[.1em] font-bold mb-2">Forex (USD)</div><div className="space-y-2">{[{l:'EUR/USD',v:forex?.rates?.EUR?(1/forex.rates.EUR).toFixed(4):'...',c:'text-sky-400'},{l:'USD/JPY',v:forex?.rates?.JPY?forex.rates.JPY.toFixed(2):'...',c:'text-orange-400'},{l:'GBP/USD',v:forex?.rates?.GBP?(1/forex.rates.GBP).toFixed(4):'...',c:'text-cyan-400'},{l:'USD/CHF',v:forex?.rates?.CHF?forex.rates.CHF.toFixed(4):'...',c:'text-emerald-400'},{l:'USD/CNY',v:forex?.rates?.CNY?forex.rates.CNY.toFixed(2):'...',c:'text-rose-400'}].map((p,i)=>(<div key={i} className="flex items-center gap-2 text-[10px]"><span className="w-16 text-white/30 truncate">{p.l}</span><div className="flex-1 h-3 rounded-full bg-white/[0.04] overflow-hidden"><div className={`h-full rounded-full ${p.c} opacity-60`} style={{width:`${Math.min(100,Math.abs(Number(p.v)||1)*20)}%`}}/></div><span className={`w-14 text-right font-bold tabular-nums ${p.c}`}>{p.v}</span></div>))}</div></div>
            {/* Market Movers */}
            <div className="rounded-2xl border border-white/[0.06] bg-white/[0.01] overflow-hidden"><div className="px-4 py-2.5 border-b border-white/[0.04] bg-white/[0.015]"><span className="text-[10px] text-cyan-400 uppercase tracking-[.15em] font-bold">Market Movers</span></div><div className="divide-y divide-white/[0.02]">{patents?.marketMovers?.length?patents.marketMovers.map((m:any,i:number)=>(<div key={i} className="px-4 py-2 flex justify-between text-[10px]"><span className="text-white/50 truncate mr-2">{m.name}</span><span className="flex gap-2 flex-shrink-0"><span className="text-white/60 tabular-nums">{m.value}</span><span className={m.dir==='up'?'text-emerald-400':'text-red-400'}>{m.change}</span></span></div>)):<div className="px-4 py-6 text-[10px] text-white/15 italic text-center">Loading...</div>}</div></div>
          </div>
          {/* Patent Panel — merged Top Holders + Valuation, no repeating names */}
          {patents&&(<div className="rounded-2xl border border-white/[0.06] bg-white/[0.01] overflow-hidden"><div className="px-4 py-2.5 border-b border-white/[0.04] bg-white/[0.015] flex items-center gap-2"><span className="text-[10px] text-pink-400 uppercase tracking-[.15em] font-bold">Patents</span><span className="text-[9px] text-white/20">{patents.header.uspto} grants · {patents.header.yoy}</span></div><div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4"><div><div className="text-[9px] text-white/15 uppercase tracking-[.1em] mb-1.5">Top Firms</div><div className="space-y-1">{patents.topHolders.map((h:any,i:number)=>(<div key={i} className="flex items-center gap-1 text-[10px]"><span className="text-white/75 w-14 truncate font-medium">{h.name}</span><span className="text-white/50 tabular-nums">{h.count}</span><span className="text-white/30">·</span><span className="text-white/40 tabular-nums">{h.mcap}</span></div>))}</div></div><div><div className="text-[9px] text-white/15 uppercase tracking-[.1em] mb-1.5">Tech Areas</div><div className="space-y-1">{patents.techAreas.map((t:any,i:number)=>(<div key={i} className="flex justify-between text-[10px]"><span className="text-white/45 truncate mr-2">{t.name}</span><span className="text-white/30">{t.pct}</span></div>))}</div></div><div><div className="text-[9px] text-white/15 uppercase tracking-[.1em] mb-1.5">Hot Areas</div><div className="space-y-1">{patents.hotAreas.map((h:any,i:number)=>(<div key={i} className="flex justify-between text-[10px]"><span className="text-white/50 truncate mr-2">{h.name}</span><span className={`text-[8px] px-1 rounded ${h.trend==='rapid'?'bg-emerald-500/15 text-emerald-400':h.trend==='growing'?'bg-amber-500/15 text-amber-400':'bg-white/5 text-white/30'}`}>{h.trend}</span></div>))}</div></div></div></div>)}
          {/* Key Labs & Institutions — RSS feed of latest posts */}
          {patents&&(()=>{const kw=patents.keyLabs.flatMap((l:any)=>[l.name,...l.name.split(/[\s-]+/).filter((s:string)=>s.length>2)]).map((s:string)=>s.toLowerCase());const labItems=items.filter((it:any)=>{const t=(it.title+' '+it.source).toLowerCase();return kw.some((k:string)=>t.includes(k));}).slice(0,12);return(<div className="rounded-2xl border border-white/[0.06] bg-white/[0.01] overflow-hidden"><div className="px-4 py-2.5 border-b border-white/[0.04] bg-white/[0.015] flex items-center justify-between"><span className="text-[10px] text-violet-400 uppercase tracking-[.15em] font-bold">Key Labs & Institutions</span><span className="text-[9px] text-white/20">{labItems.length} posts</span></div><div className="divide-y divide-white/[0.02] max-h-[220px] overflow-y-auto scrollbar-hide">{labItems.length===0?<div className="px-4 py-6 text-[10px] text-white/15 italic text-center">Awaiting signal matching...</div>:labItems.map((it:any,j:number)=>(<a key={j} href={it.url} target="_blank" rel="noopener noreferrer" className="block px-4 py-2.5 hover:bg-white/[0.03] group"><div className="text-[11px] font-medium text-white/60 group-hover:text-white/85 line-clamp-2 leading-snug">{it.title}</div><div className="flex items-center gap-2 mt-1 text-[9px] text-white/20"><span className="truncate max-w-[100px]">{it.source}</span><span className="ml-auto tabular-nums">{ago(it.published_at)}</span></div></a>))}</div></div>)})()}
          {/* Category boxes */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{macroCats.map(cat=>(<div key={cat.id} className={`rounded-2xl border border-white/[0.06] ${cat.bg} border-l-2 ${cat.color} overflow-hidden`}><div className="px-4 py-3 border-b border-white/[0.04] bg-white/[0.015] flex items-center justify-between"><span className={`text-[13px] font-semibold ${cat.accent}`}>{cat.label}</span><span className="text-[10px] px-2 py-0.5 rounded bg-white/[0.06] text-white/30 tabular-nums">{cat.count}</span></div><div className="divide-y divide-white/[0.02] max-h-[260px] overflow-y-auto scrollbar-hide">{cat.items.length===0?<div className="px-4 py-6 text-[11px] text-white/10 italic text-center">no signals</div>:cat.items.map((it,j)=>(<a key={j} href={it.url} target="_blank" rel="noopener noreferrer" className="block px-4 py-2.5 hover:bg-white/[0.03] group"><div className="text-[11px] font-medium text-white/60 group-hover:text-white/85 line-clamp-1">{it.title}</div><div className="flex items-center gap-2 mt-1 text-[9px] text-white/20"><span className="truncate max-w-[80px]">{it.source}</span><span className="ml-auto tabular-nums">{ago(it.published_at)}</span></div></a>))}</div></div>))}</div>
        </div>)}

        {/* ============ INFOSEC TAB ============ */}
        {active==='infosec'&&(<div className="space-y-5">
          <div className="rounded-2xl border border-red-500/30 bg-red-500/[0.03] p-5"><div className="flex items-center gap-2 mb-3"><span className="text-[11px] text-red-400 uppercase tracking-[.15em] font-bold">⚠ Active Threats</span><span className="w-1.5 h-1.5 rounded-full bg-red-400 animate-pulse"/></div><div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-[12px]">{dd2?.kev?.length>0?<div className="rounded-xl border border-red-500/10 bg-red-500/[0.02] p-3"><div className="font-semibold text-red-300 mb-1">🔐 {dd2.kev.length} KEV Active</div><div className="text-white/50">CISA Known Exploited. Apply patches within due dates.</div></div>:<div className="rounded-xl border border-red-500/10 bg-red-500/[0.02] p-3"><div className="font-semibold text-red-300 mb-1">🔐 Revoke</div><div className="text-white/50">Check wallet approvals. Use revoke.cash</div></div>}{dd2?.cves?.filter((c:any)=>c.severity==='CRITICAL'||c.severity==='HIGH').length>0?<div className="rounded-xl border border-orange-500/10 bg-orange-500/[0.02] p-3"><div className="font-semibold text-orange-300 mb-1">📦 {dd2.cves.filter((c:any)=>c.severity==='CRITICAL'||c.severity==='HIGH').length} Critical CVEs</div><div className="text-white/50">Update affected systems immediately.</div></div>:<div className="rounded-xl border border-orange-500/10 bg-orange-500/[0.02] p-3"><div className="font-semibold text-orange-300 mb-1">📦 Update</div><div className="text-white/50">Run audit. Patch compromised packages.</div></div>}{dd2?.breaches?.length>0?<div className="rounded-xl border border-yellow-500/10 bg-yellow-500/[0.02] p-3"><div className="font-semibold text-yellow-300 mb-1">🛡️ {dd2.breaches.length} Breaches</div><div className="text-white/50">Change passwords. Enable 2FA on exposed accounts.</div></div>:<div className="rounded-xl border border-yellow-500/10 bg-yellow-500/[0.02] p-3"><div className="font-semibold text-yellow-300 mb-1">🛡️ Check</div><div className="text-white/50">Audit exposed ports. Rotate keys...</div></div>}</div></div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <TileBox title="CISA KEV" accent="text-red-400" color="border-l-red-400" count={dd2?.kev?.length||0}>{(dd2?.kev||[]).map((v:any,j:number)=>(<div key={j} className="px-4 py-2.5 border-b border-white/[0.02] last:border-0"><div className="text-[12px] font-medium text-white/70">{v.cve}</div><div className="text-[10px] text-white/30 mt-0.5 line-clamp-2">{v.vendor} — {v.product}: {v.name}</div><div className="text-[9px] text-red-400/60 mt-1">Due: {v.dueDate?.slice(0,10)}</div></div>))}</TileBox>
            <TileBox title="Latest CVEs" accent="text-orange-400" color="border-l-orange-400" count={dd2?.cves?.length||0}>{(dd2?.cves||[]).map((c:any,j:number)=>(<div key={j} className="px-4 py-2.5 border-b border-white/[0.02] last:border-0"><div className="flex items-center gap-2"><span className="text-[12px] font-medium text-white/70">{c.id}</span><SeverityBadge sev={c.severity} score={c.score}/></div><div className="text-[10px] text-white/30 mt-0.5 line-clamp-2">{c.description}</div></div>))}</TileBox>
            <TileBox title="Recent Breaches" accent="text-pink-400" color="border-l-pink-400" count={dd2?.breaches?.length||0}>{(dd2?.breaches||[]).map((b:any,j:number)=>(<div key={j} className="px-4 py-2.5 border-b border-white/[0.02] last:border-0"><div className="text-[12px] font-medium text-white/70">{b.name}</div><div className="text-[10px] text-white/30 mt-0.5">{b.domain} · {b.count?.toLocaleString()} accounts · {b.data}</div><div className="text-[9px] text-white/15 mt-0.5">{b.date}</div></div>))}</TileBox>
          </div>
          {/* Social Media */}
        ...
        {active==='web3'&&(<div className="space-y-5">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <div className="lg:col-span-1 rounded-2xl border border-white/[0.06] bg-white/[0.01] overflow-hidden"><div className="px-5 py-3 border-b border-white/[0.04] bg-white/[0.015]"><div className="text-[11px] text-purple-400 uppercase tracking-[.15em] font-bold">TVL by Chain</div></div><div className="grid grid-cols-[28px_1fr_70px] gap-3 px-5 py-2.5 border-b border-white/[0.03] bg-white/[0.02] text-[10px] text-white/20 uppercase tracking-wider font-semibold"><div>#</div><div>Chain</div><div className="text-right">TVL</div></div>{(dd?.tvl||[]).slice(0,12).map((c:any,i:number)=>(<div key={i} className={`grid grid-cols-[28px_1fr_70px] gap-3 px-5 py-2.5 border-b border-white/[0.02] last:border-0 hover:bg-white/[0.02] ${i%2===1?'bg-white/[0.005]':''}`}><div className="text-[11px] text-white/25 tabular-nums">{i+1}</div><div className="text-[12px] font-medium text-white/75 truncate">{c.name}</div><div className="text-[12px] font-semibold text-right tabular-nums text-purple-400/80">{fmt(c.tvl)}</div></div>))||<div className="p-5 text-white/15 text-xs italic text-center">Loading...</div>}</div>
            <div className="lg:col-span-2 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.01] p-5"><div className="text-[10px] text-cyan-400 uppercase tracking-[.15em] font-bold mb-2">Total Volume</div><div className="text-2xl font-bold tabular-nums text-white/80">{fmt(totalVol)}</div><div className="text-[10px] text-white/20 mt-1">24h DEX</div><div className="mt-3 space-y-1">{(dd?.volume||[]).slice(0,5).map((v:any,i:number)=>(<div key={i} className="flex justify-between text-[11px]"><span className="text-white/30 truncate">{v.name}</span><span className="text-white/50 tabular-nums">{fmt(v.volume24h)}</span></div>))}</div></div>
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.01] p-5"><div className="text-[10px] text-emerald-400 uppercase tracking-[.15em] font-bold mb-3">Stablecoins</div><div className="space-y-2">{(dd?.stablecoins||[]).map((s:any,i:number)=>(<div key={i} className="flex justify-between"><span className="text-[12px] text-white/50">{s.name}</span><span className="text-[11px] text-white/70 tabular-nums">{fmt(s.circulating)}</span></div>))||<div className="text-white/15 text-xs italic">Loading...</div>}</div></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.01] p-5"><div className="text-[10px] text-amber-400 uppercase tracking-[.15em] font-bold mb-3">Fees (24h)</div><div className="space-y-2">{(dd?.fees||[]).slice(0,6).map((f:any,i:number)=>(<div key={i} className="flex justify-between"><span className="text-[12px] text-white/50 truncate">{f.name}</span><span className="text-[11px] text-white/70 tabular-nums">{fmt(f.fees24h)}</span></div>))||<div className="text-white/15 text-xs italic">Loading...</div>}</div></div>
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.01] p-5 flex flex-col items-center"><div className="text-[10px] text-amber-400 uppercase tracking-[.15em] font-bold mb-3">F&G</div><div className="relative h-32 w-6 bg-white/[0.04] rounded-full overflow-hidden mb-2"><div className={`absolute bottom-0 w-full rounded-full ${fgVal>50?'bg-emerald-500/60':fgVal<30?'bg-red-500/60':'bg-amber-500/60'}`} style={{height:`${Math.max(3,fgVal)}%`}}/></div><div className={`text-xl font-bold mt-2 ${fgVal>50?'text-emerald-400':fgVal<30?'text-red-400':'text-amber-400'}`}>{fgVal||'--'}</div><div className="text-[10px] text-white/25 mt-0.5">{fgLabel||'...'}</div></div>
              </div>
              {dd?.polymarket?.length>0&&(<div className="rounded-2xl border border-white/[0.06] bg-white/[0.01] overflow-hidden"><div className="px-5 py-3 border-b border-white/[0.04] bg-white/[0.015]"><div className="text-[11px] text-pink-400 uppercase tracking-[.15em] font-bold">Polymarket</div></div><div className="grid grid-cols-[1fr_70px_70px] gap-3 px-5 py-2.5 border-b border-white/[0.03] bg-white/[0.02] text-[10px] text-white/20 uppercase tracking-wider font-semibold"><div>Market</div><div className="text-right">Vol</div><div className="text-right">Liq</div></div>{dd.polymarket.slice(0,5).map((m:any,i:number)=>(<a key={i} href={`https://polymarket.com/event/${m.slug}`} target="_blank" rel="noopener noreferrer" className={`grid grid-cols-[1fr_70px_70px] gap-3 px-5 py-2.5 border-b border-white/[0.02] last:border-0 hover:bg-white/[0.02] ${i%2===1?'bg-white/[0.005]':''}`}><div className="text-[12px] text-white/60 hover:text-white/80 truncate">{m.title}</div><div className="text-[11px] text-right text-white/30 tabular-nums">{fmtN(m.volume)}</div><div className="text-[11px] text-right text-white/20 tabular-nums">{fmtN(m.liquidity)}</div></a>))}</div>)}
            </div>
          </div>
          {/* Category boxes */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{web3Cats.map(cat=>(<div key={cat.id} className={`rounded-2xl border border-white/[0.06] ${cat.bg} border-l-2 ${cat.color} overflow-hidden`}><div className="px-4 py-3 border-b border-white/[0.04] bg-white/[0.015] flex items-center justify-between"><span className={`text-[13px] font-semibold ${cat.accent}`}>{cat.label}</span><span className="text-[10px] px-2 py-0.5 rounded bg-white/[0.06] text-white/30 tabular-nums">{cat.count}</span></div><div className="divide-y divide-white/[0.02] max-h-[260px] overflow-y-auto scrollbar-hide">{cat.items.length===0?<div className="px-4 py-6 text-[11px] text-white/10 italic text-center">no signals</div>:cat.items.map((it,j)=>(<a key={j} href={it.url} target="_blank" rel="noopener noreferrer" className="block px-4 py-2.5 hover:bg-white/[0.03] group"><div className="text-[11px] font-medium text-white/60 group-hover:text-white/85 line-clamp-1">{it.title}</div><div className="flex items-center gap-2 mt-1 text-[9px] text-white/20"><span className="truncate max-w-[80px]">{it.source}</span><span className="ml-auto tabular-nums">{ago(it.published_at)}</span></div></a>))}</div></div>))}</div>
        </div>)}
      </div>
    </div>
  );
