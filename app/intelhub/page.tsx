...
const KEYWORDS: Record<string, string> = {
  physics:'physics,quantum,particle,fusion,astrophysics',energy:'energy,nuclear,solar,wind,grid,battery,oil,gas,opec',
  space:'space,spacex,nasa,rocket,satellite,orbital',cybersec:'cybersec,cve,vulnerability,breach,ransomware,outage,bgp,ddos,cisa,kev,zero-day,exploit,malware,hibp',
  opsec:'opsec,hardening,threat model,privacy,air-gapped',crypto:'bitcoin,ethereum,token,defi,yield,polymarket,orderbook,perp,stablecoin,staking,validator,dex,l2,rollup',
  web3:'web3,blockchain,evm,on-chain,dao,nft,smart contract',macro:'macro,geopolitic,sanction,ofac,tariff,federal reserve,inflation,gdp,rate hike,treasury,sec',
  ai:'llm,transformer,gpt,claude,deepseek,sovereign ai,lora,qlora',ml:'machine learning,ml,deep learning,training,benchmark,fine-tuning',
  hardware:'chip,gpu,cpu,npu,tpu,h100,compute cluster,meshtastic',ip:'patent,uspto,trademark,copyright',
...
function rel(it:{title:string;source:string}){if(it.source?.toLowerCase().includes('polymarket')){const t=it.title.toLowerCase();if(POLY_JUNK.some(k=>t.includes(k)))return false;return POLY_KEEP.some(k=>t.includes(k));}if(it.source?.toLowerCase().includes('hacker news')||it.source?.toLowerCase().includes('y combinator'))return!JUNK_HN.some(p=>p.test(it.title));return true;}

/* ================================================================ */
interface Item{title:string;url:string;source:string;published_at:string;summary:string;tag?:string}
interface Chain{name:string;tvl:number;symbol:string}
interface DexVol{name:string;volume24h:number}
interface PolyMkt{title:string;slug:string;volume:number;liquidity:number;url:string}

export default function IntelHubPage(){
  const [items,setItems]=useState<Item[]>([]);
  const [loading,setLoading]=useState(true);
  const [active,setActive]=useState<'macro'|'infosec'|'web3'>('web3');
  const [md,setMd]=useState<{topChains:Chain[];topDex:DexVol[];totalVolume24h:number;polyMarkets:PolyMkt[]}|null>(null);
  const scrollRef=useRef<HTMLDivElement>(null);
  const speed=useRef(1.2);
  const af=useRef(0);

  const loadIntel=async()=>{try{const r=await fetch('/api/intel/raw-items');if(!r.ok)return;const d=await r.json();if(Array.isArray(d))setItems(d.map((x:any)=>({...x,tag:getTag(x.title||'',x.summary||'')})).filter(rel));}catch(e){console.error(e);}};
  const loadMarket=async()=>{try{const r=await fetch('/api/intel/market-data');if(r.ok)setMd(await r.json());}catch(e){}};

  useEffect(()=>{setLoading(true);Promise.all([loadIntel(),loadMarket()]).finally(()=>setLoading(false));const i=setInterval(()=>{loadIntel();loadMarket();},5*60_000);return()=>clearInterval(i);},[]);
  ...
  const fmt=(n:number)=>{if(n>=1e9)return`$${(n/1e9).toFixed(2)}B`;if(n>=1e6)return`$${(n/1e6).toFixed(2)}M`;if(n>=1e3)return`$${(n/1e3).toFixed(2)}K`;return`$${n.toFixed(2)}`;};

  const dd=DASH[active];const dc=dd;
  const top3=items.slice(0,3);

  const sourceTiles=dd.sources.map(s=>{const m=items.filter(i=>s.match.some(k=>i.source?.toLowerCase().includes(k)));return{...s,latest:m[0],count:m.length,items:m.slice(0,3)};});
    ...
    <div className="min-h-screen bg-[#040407] text-white"><Navbar/>
      ...
      <div className="max-w-[1440px] mx-auto px-8 pb-24">
        <div className="flex gap-1 bg-white/[0.02] p-1 rounded-2xl w-fit mb-8 border border-white/[0.04]">
          {(['macro','infosec','web3']as const).map(d=>{const s=DASH[d];return(<button key={d} onClick={()=>setActive(d)} className={`px-6 py-2.5 rounded-xl text-[13px] font-medium transition-all duration-200 ${active===d?`${s.accent} bg-white/[0.08] shadow-sm`:'text-white/25 hover:text-white/50'}`}>{d==='macro'?'Macro':d==='infosec'?'Infosec':'Web3'}</button>);})}
        </div>

        {/* ======== WEB3: Market Data Panels ======== */}
        {active==='web3'&&md&&(
          <div className="mb-10 space-y-8">
            {/* TVL Leaderboard */}
            <div>
              <div className="flex items-center gap-2 mb-4"><span className="text-[11px] text-purple-400 uppercase tracking-[.15em] font-bold">TVL by Chain</span><span className="text-[10px] text-white/20">DeFiLlama</span></div>
              <div className="rounded-2xl border border-white/[0.06] overflow-hidden bg-white/[0.01]">
                <div className="grid grid-cols-[40px_1fr_120px] gap-3 px-5 py-3 border-b border-white/[0.04] bg-white/[0.015] text-[11px] text-white/20 uppercase tracking-wider font-semibold"><div>#</div><div>Chain</div><div className="text-right">TVL</div></div>
                {md.topChains.map((c,i)=>(
                  <div key={i} className={`grid grid-cols-[40px_1fr_120px] gap-3 px-5 py-3 border-b border-white/[0.03] last:border-0 ${i%2===1?'bg-white/[0.008]':''}`}>
                    <div className="text-[12px] text-white/30 tabular-nums">{i+1}</div>
                    <div className="text-[13px] font-medium text-white/80">{c.name}</div>
                    <div className="text-[13px] font-semibold text-right tabular-nums text-emerald-400/80">{fmt(c.tvl)}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-4"><span className="text-[11px] text-cyan-400 uppercase tracking-[.15em] font-bold">DEX Daily Volume</span><span className="text-[10px] text-white/20">DeFiLlama</span></div>
              <div className="rounded-2xl border border-white/[0.06] overflow-hidden bg-white/[0.01]">
                <div className="grid grid-cols-[40px_1fr_120px] gap-3 px-5 py-3 border-b border-white/[0.04] bg-white/[0.015] text-[11px] text-white/20 uppercase tracking-wider font-semibold"><div>#</div><div>Chain</div><div className="text-right">24h Volume</div></div>
                {md.topDex.slice(0,10).map((c,i)=>(
                  <div key={i} className={`grid grid-cols-[40px_1fr_120px] gap-3 px-5 py-3 border-b border-white/[0.03] last:border-0 ${i%2===1?'bg-white/[0.008]':''}`}>
                    <div className="text-[12px] text-white/30 tabular-nums">{i+1}</div>
                    <div className="text-[13px] font-medium text-white/80">{c.name}</div>
                    <div className="text-[13px] font-semibold text-right tabular-nums text-cyan-400/80">{fmt(c.volume24h)}</div>
                  </div>
                ))}
                {md.totalVolume24h>0&&(
                  <div className="px-5 py-3 border-t border-white/[0.04] bg-white/[0.015] flex justify-between text-[11px]"><span className="text-white/20">Total 24h</span><span className="font-semibold tabular-nums text-cyan-400">{fmt(md.totalVolume24h)}</span></div>
                )}
              </div>
            </div>
            ...
            {/* Polymarket Odds */}
            <div>
              <div className="flex items-center gap-2 mb-4"><span className="text-[11px] text-pink-400 uppercase tracking-[.15em] font-bold">Polymarket</span><span className="text-[10px] text-white/20">crypto & macro markets</span></div>
              <div className="rounded-2xl border border-white/[0.06] overflow-hidden bg-white/[0.01]">
                <div className="grid grid-cols-[1fr_90px_90px] gap-3 px-5 py-3 border-b border-white/[0.04] bg-white/[0.015] text-[11px] text-white/20 uppercase tracking-wider font-semibold"><div>Market</div><div className="text-right">Volume</div><div className="text-right">Liquidity</div></div>
                {md.polyMarkets.map((m,i)=>(
                  <a key={i} href={m.url} target="_blank" rel="noopener noreferrer" className={`grid grid-cols-[1fr_90px_90px] gap-3 px-5 py-3 border-b border-white/[0.03] last:border-0 hover:bg-white/[0.03] transition-all ${i%2===1?'bg-white/[0.008]':''}`}>
                    <div className="text-[13px] font-medium text-white/70 hover:text-white/90 truncate">{m.title}</div>
                    <div className="text-[12px] text-right tabular-nums text-white/40">{fmt(m.volume)}</div>
                    <div className="text-[12px] text-right tabular-nums text-white/25">{fmt(m.liquidity)}</div>
                  </a>
                ))}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {sourceTiles.map((src,i)=>(
            <div key={i} className={`rounded-2xl border border-white/[0.06] bg-white/[0.01] border-l-2 ${src.color} overflow-hidden hover:border-white/15 transition-all duration-300`}>
              <div className="px-4 py-3 border-b border-white/[0.04] flex items-center justify-between bg-white/[0.015]"><div className="flex items-center gap-2"><span className="text-[13px] font-semibold text-white/80">{src.name}</span>{src.count>0&&<span className="text-[10px] px-1.5 py-0.5 rounded bg-white/[0.06] text-white/30">{src.count}</span>}</div><div className="flex items-center gap-2 text-[10px] text-white/30">{src.latest?<span className="flex items-center gap-1"><span className={`w-1.5 h-1.5 rounded-full ${isNew(src.latest.published_at)?'bg-emerald-400':'bg-white/20'}`}/>{ago(src.latest.published_at)} ago</span>:<span className="text-white/15 italic">no data</span>}</div></div>
              <div className="divide-y divide-white/[0.03]">{src.items.length===0?<div className="px-4 py-4 text-[12px] text-white/15 italic text-center">Awaiting data</div>:src.items.map((it,j)=>(<a key={j} href={it.url} target="_blank" rel="noopener noreferrer" className="block px-4 py-3 hover:bg-white/[0.03] transition-all duration-150 group"><div className="text-[12px] font-medium text-white/70 group-hover:text-white/90 line-clamp-1 leading-snug">{it.title}</div><div className="flex items-center gap-2 mt-1.5 text-[10px] text-white/25">{it.tag&&<span className={`px-1.5 py-0.5 rounded font-semibold text-[9px] ${TC[it.tag]||''}`}>#{it.tag}</span>}<span className="ml-auto tabular-nums">{ago(it.published_at)}</span></div></a>))}</div>
            </div>
          ))}
        </div>
