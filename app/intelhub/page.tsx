...
/* ================================================================
   CATEGORIES — ordered: important first, avoid greedy matches
   ================================================================ */
const KEYWORDS: Record<string, string> = {
  ...
  macro: 'macro,geopolitic,sanction,ofac,tariff,export control,trade war,federal reserve,bis,un security,gdp,inflation,rate hike,treasury,sec,regulation',
...
const TC: Record<string, string> = {
  ...
  ip:'bg-pink-500/15 text-pink-400', prediction:'bg-cyan-500/15 text-cyan-400',
  research:'bg-indigo-500/15 text-indigo-400', legal:'bg-rose-500/15 text-rose-400',
  physics:'bg-violet-500/15 text-violet-400', chemistry:'bg-teal-500/15 text-teal-400',
  biology:'bg-emerald-500/15 text-emerald-400', energy:'bg-lime-500/15 text-lime-400',
  space:'bg-fuchsia-500/15 text-fuchsia-400', med:'bg-rose-500/15 text-rose-400',
};

const DASH: Record<string, { accent: string; title: string; sources: string[] }> = {
  macro: {
    accent: 'text-amber-400', title: 'Macro & Geopolitical',
    sources: ['OFAC','BIS','ITA','UN','Reuters','Federal Reserve','IMF','World Bank','Brookings','CFR','Sanctions','Trade','Tariff','Export','Treasury','SEC'],
  },
  infosec: {
    accent: 'text-orange-400', title: 'Cybersecurity & Infrastructure',
    sources: ['CISA','ICS-CERT','NIST','HIBP','Krebs','Hacker News','BleepingComputer','Dark Reading','Schneier','Vulnerability','CVE','Breach','Outage','Malware','Ransomware'],
  },
  web3: {
    accent: 'text-purple-400', title: 'Crypto & DeFi',
    sources: ['CoinDesk','The Block','Defiant','Decrypt','Bankless','DeFiLlama','Polymarket','IPFS','Protocol Labs','Filecoin','Yield','Token','Volume','DEX','Bitcoin','Ethereum'],
  },
};
...
export default function IntelHubPage() {
  ...
  const di=items.filter(i=>ds.sources.some(s=>i.source?.toLowerCase().includes(s.toLowerCase())||i.tag&&ds.sources.some(s2=>s2.toLowerCase()===i.tag))).slice(0,15);
  ...
  return (
    <div className="min-h-screen bg-[#040407] text-white"><Navbar/>
      ...
      <div className="border-b border-white/[0.04] py-5 bg-[#080810]">
        <div className="max-w-[1440px] mx-auto px-8">
          ...
          <div ref={scrollRef} className="flex gap-3" style={{overflowX:'scroll',scrollbarWidth:'none',WebkitMaskImage:'linear-gradient(to right, transparent 0%, black 3%, black 97%, transparent 100%)',maskImage:'linear-gradient(to right, transparent 0%, black 3%, black 97%, transparent 100%)'}}>
            {loading&&Array.from({length:6}).map((_,i)=>(<div key={i} className="flex-shrink-0 w-[300px] rounded-2xl p-4 bg-white/[0.02] border border-white/[0.04] animate-pulse"><div className="h-3 bg-white/[0.05] rounded w-3/4 mb-3"/><div className="h-3 bg-white/[0.05] rounded w-1/2"/><div className="flex gap-2 mt-4"><div className="h-2.5 bg-white/[0.05] rounded w-12"/><div className="h-2.5 bg-white/[0.05] rounded w-16"/></div></div>))}
      ...
      <div className="max-w-[1440px] mx-auto px-8 pb-24">
        ...
        <div className="flex items-center justify-between mb-4"><div className="flex items-center gap-3"><span className={`text-[12px] uppercase tracking-[.2em] font-bold ${dc.accent}`}>{dc.title}</span><span className="w-px h-3 bg-white/[0.06]"/><span className="text-[11px] text-white/15">{di.length} high signal</span></div></div>
        ...
        <div className="mt-6"><div className="flex items-center gap-3 mb-4"><span className="text-[10px] text-white/15 uppercase tracking-[.15em]">Card View</span></div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {di.slice(0,6).map((it,i)=>(<a key={i} href={it.url} target="_blank" rel="noopener noreferrer" className="group rounded-2xl p-4 border border-white/[0.05] hover:border-white/15 bg-white/[0.01] hover:bg-white/[0.04] transition-all duration-300"><div className="text-[13px] font-semibold leading-snug text-white/80 group-hover:text-white line-clamp-2">{it.title}</div><div className="flex items-center gap-2 mt-3 text-[11px] text-white/25">{it.tag&&<span className={`px-2 py-0.5 rounded-md font-semibold text-[10px] ${TC[it.tag]?.split(' ')[0]||''} ${TC[it.tag]?.split(' ')[1]||''}`}>#{it.tag}</span>}<span>{it.source}</span><span className="ml-auto tabular-nums">{ago(it.published_at)}</span></div></a>))}
          </div>
        </div>
      </div>
    </div>
