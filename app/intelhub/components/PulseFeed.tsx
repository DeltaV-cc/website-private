...
export default function PulseFeed({
  ...
  return (
    <div className="border-b border-[var(--border-default)] py-4 bg-[var(--surface-card)]">
      <div className="max-w-[1440px] mx-auto px-6 md:px-8">
        <div className="flex items-center gap-3 mb-3">
          ...
          <span className="w-px h-3 bg-[var(--overlay-soft)]" />
        ...
        >
          {loading &&
            Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="flex-shrink-0 w-[260px] rounded-2xl p-4 bg-[var(--bg-surface)] border border-[var(--border-default)]">
                <div className="h-3 bg-[var(--overlay-soft)] rounded w-3/4 mb-3" />
                <div className="h-3 bg-[var(--overlay-soft)] rounded w-1/2" />
              </div>
            ))}
          ...
          {items.map((it, i) => (
            <a
              ...
              className={`flex-shrink-0 w-[260px] rounded-2xl p-4 border border-[var(--border-default)] bg-[var(--bg-surface)] hover:bg-[var(--overlay-soft)] hover:border-[var(--border-subtle)] transition-colors duration-200 group ${it.tag ? (BCOL[it.tag] || 'border-l-[var(--border-subtle)]') : 'border-l-[var(--border-subtle)]'} border-l-2`}
            >
              <div className="flex items-start gap-2">
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium leading-snug line-clamp-2 text-[var(--text-primary)] group-hover:text-[var(--text-primary)]">{it.title}</div>
