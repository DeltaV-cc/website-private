  ...
  return (
    <main className="page-container pt-16 md:pt-20 pb-24 max-w-3xl">
      ...
      <div className="mt-14 flex flex-wrap justify-between gap-4 border-t border-[var(--border-default)] pt-8">
        {prev ? (
          <Link
            href={`/forge/course/my-first-ai-agent/labs/${prev.slug}/`}
            className="text-sm text-[var(--text-secondary)] hover:text-[var(--course-accent)]"
          >
            ← {prev.title}
          </Link>
        ) : (
          <Link href="/forge/course/my-first-ai-agent/labs/" className="text-sm text-[var(--text-secondary)]">
            ← All labs
          </Link>
        )}
        {next ? (
          <Link
            href={`/forge/course/my-first-ai-agent/labs/${next.slug}/`}
