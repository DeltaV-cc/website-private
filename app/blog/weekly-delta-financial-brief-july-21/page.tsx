      ...
      <div className="bg-[var(--bg-elevated)] border border-[var(--border-default)] rounded-2xl p-5 my-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            ...
            <div className="text-sm font-semibold text-[var(--text-primary)]">$64,176</div>
            <div className="text-[10px] text-[var(--accent-red)] mt-0.5">−0.66%</div>
          </div>
          <div>
            <div className="text-[10px] text-[var(--text-muted)] uppercase tracking-[1px] mb-1">ETH</div>
            <div className="text-sm font-semibold text-[var(--text-primary)]">$1,861</div>
            <div className="text-[10px] text-[var(--accent-red)] mt-0.5">−0.29%</div>
          </div>
          <div>
            <div className="text-[10px] text-[var(--text-muted)] uppercase tracking-[1px] mb-1">Nasdaq 100</div>
            <div className="text-sm font-semibold text-[var(--text-primary)]">−4.1% WoW</div>
            <div className="text-[10px] text-[var(--accent-red)] mt-0.5">Risk-off rotation</div>
          </div>
          <div>
            <div className="text-[10px] text-[var(--text-muted)] uppercase tracking-[1px] mb-1">Stablecoin MCap</div>
            <div className="text-sm font-semibold text-[var(--text-primary)]">$309.4B</div>
            <div className="text-[10px] text-[var(--text-muted)] mt-0.5">Flat WoW</div>
          </div>
        </div>
      ...
      <div className="bg-[var(--bg-elevated)] border border-[var(--border-default)] rounded-2xl p-5 my-4">
        <div className="border-l-2 border-[var(--accent-red)] pl-4">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold text-[var(--text-primary)]">Bonzo Finance — $9.05M</span>
            <span className="text-[10px] px-2 py-0.5 rounded bg-[var(--accent-red)]/15 text-[var(--accent-red)]">Oracle Exploit</span>
            <span className="text-[10px] text-[var(--text-muted)]">July 14</span>
          </div>
          <p className="text-xs text-[var(--text-muted)]">Supra oracle verifier accepted a zeroed signature against a zeroed key on Hedera. $9M gone because "the math checked out." Classic oracle integration failure — the verifier didn't validate that the key was non-zero. Every protocol using third-party oracles should audit this specific vector.
          </p>
        </div>
        <div className="mt-4 p-3 rounded-xl bg-[var(--bg-elevated)] border border-[var(--border-default)]">
          <p className="text-xs text-[var(--text-tertiary)]">
            <strong className="text-[var(--text-primary)]">Takeaway:</strong> oracle signature verification must include a non-zero key check. This is not a new vulnerability class — it's a failure of integration testing. Assume third-party components will accept malformed input until proven otherwise.
          </p>
        </div>
      ...
      <div className="bg-[var(--bg-elevated)] border border-[var(--border-default)] rounded-2xl p-5 my-4">
        <div className="grid md:grid-cols-3 gap-3 mb-4">
          <div className="bg-[var(--bg-card)] rounded-xl p-3 border border-[var(--border-default)]">
            ...
            <div className="text-[10px] text-[var(--text-muted)]">Flat WoW</div>
          </div>
          <div className="bg-[var(--bg-card)] rounded-xl p-3 border border-[var(--border-default)]">
            <div className="text-[10px] text-[var(--text-muted)] uppercase tracking-[1px]">USDT Dominance</div>
            <div className="text-lg font-mono text-[var(--accent-cyan)]">$184B</div>
            <div className="text-[10px] text-[var(--text-muted)]">59.5% of market</div>
          </div>
          <div className="bg-[var(--bg-card)] rounded-xl p-3 border border-[var(--border-default)]">
            <div className="text-[10px] text-[var(--text-muted)] uppercase tracking-[1px]">Sky Dollar (USDS)</div>
            <div className="text-lg font-mono text-[var(--accent-red)]">$6.66B</div>
            <div className="text-[10px] text-[var(--text-muted)]">−12.15% WoW contraction</div>
          </div>
        </div>
        ...
        <p className="text-xs text-[var(--text-muted)] mt-3">Sources: DeFiLlama, Pharos Watch</p>
      </div>
      ...
      <div className="bg-[var(--bg-elevated)] border border-[var(--border-default)] rounded-2xl p-5 my-4">
        <div className="space-y-3">
          <div className="flex gap-3">
            <span className="w-2 h-2 rounded-full bg-[var(--accent-green)] mt-1.5 flex-shrink-0" />
            <div>
              <p className="text-sm text-[var(--text-primary)] font-semibold">SEC Reverses Pattern Day Trader Rule</p>
              <p className="text-xs text-[var(--text-tertiary)]">Major win for retail trading platforms. Robinhood is the primary beneficiary — expect increased trading volumes across the brokerage sector.</p>
            </div>
          </div>
          <div className="flex gap-3">
            <span className="w-2 h-2 rounded-full bg-[var(--accent-amber)] mt-1.5 flex-shrink-0" />
            <div>
              <p className="text-sm text-[var(--text-primary)] font-semibold">Morgan Stanley Launches MSBT at 0.14%</p>
              <p className="text-xs text-[var(--text-tertiary)]">First spot BTC ETF issued directly by a major U.S. bank. Over $100M in week-one flows. BlackRock's IBIT ($55B) finally has a serious banking competitor.</p>
            </div>
          </div>
          <div className="flex gap-3">
            <span className="w-2 h-2 rounded-full bg-[var(--accent-amber)] mt-1.5 flex-shrink-0" />
            <div>
              <p className="text-sm text-[var(--text-primary)] font-semibold">Schwab Introduces Spot Crypto Trading</p>
              <p className="text-xs text-[var(--text-tertiary)]">Phased rollout to 38.5M accounts. The last major brokerage domino falls — crypto is now accessible from every major U.S. brokerage platform.</p>
            </div>
          </div>
        </div>
        <p className="text-xs text-[var(--text-muted)] mt-4 pt-3 border-t border-[var(--border-default)]">Sources: CoinDesk, Congressional tracker, SEC</p>
      </div>
