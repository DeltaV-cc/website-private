...
export default function IBMSubNm() {
  return (
    ...
    >
      <p>
        The nanometer era is over. On June 25, IBM announced the <strong className="text-[var(--text-primary)]">world&apos;s first sub-1 nanometer chip</strong> — 
        a 0.7nm (7 angstrom) node with a completely new transistor architecture called nanostack. 
        The chip packs <strong className="text-[var(--text-primary)]">nearly 100 billion transistors</strong> onto a die the size of a fingernail, 
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 my-8">
        {[
          { value: '0.7nm', label: 'Node Size', color: 'text-[var(--accent-cyan)]' },
          { value: '~100B', label: 'Transistors', color: 'text-[var(--accent-purple)]' },
          { value: '+50%', label: 'Performance vs 2nm', color: 'text-[var(--accent-gold)]' },
          { value: '-70%', label: 'Energy vs 2nm', color: 'text-[var(--accent-orange)]' },
        ].map((stat) => (
      ...
      <h2 className="text-2xl font-semibold text-[var(--text-primary)] mt-10 mb-4">Nanostack — Building Chips in Three Dimensions</h2>

      <p>
        The key innovation is <strong className="text-[var(--text-primary)]">nanostack</strong> — IBM&apos;s first 3D nanosheet-based transistor 
        architecture. Instead of laying transistors flat on the silicon surface (the method used since the 
        1960s), nanostack <strong className="text-[var(--text-primary)]">vertically stacks and staggers transistors layer by layer</strong> 
      ...
      <p>
        ...
        A companion paper at VLSI 2026 showed <strong className="text-[var(--text-primary)]">40% SRAM scaling</strong> using the same 
      ...
      <div className="bg-[var(--bg-card)] border-l-2 border-[var(--accent-cyan)] rounded-r-xl p-5 my-6 text-sm">
        <p className="text-[var(--text-tertiary)]">
          <strong className="text-[var(--accent-cyan)]">Why this matters:</strong> The industry has been predicting the end of Moore&apos;s Law for 
          decades. At sub-1nm, features approach the size of individual atoms — silicon atoms are 
          about 0.2nm apart. IBM&apos;s roadmap projects <strong className="text-[var(--text-primary)]">at least another decade of scaling</strong> 
          beyond this node using nanostack.
        </p>
      </div>

      <h2 className="text-2xl font-semibold text-[var(--text-primary)] mt-10 mb-4">The Production Path — 5 Years Out</h2>

      <p>
        IBM expects the earliest adoption of nanostack at the sub-1nm node with a 
        <strong className="text-[var(--text-primary)]"> path to production in as early as 5 years</strong>. Partners include ASML 
      ...
      <p>
        IBM also announced <strong className="text-[var(--text-primary)]">Anderon</strong> — the world&apos;s first pure-play quantum foundry, 
