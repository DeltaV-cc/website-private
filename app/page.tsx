  ...
  return <main className="relative isolate">
    ...
    <section id="pillars" className="page-container py-20 md:py-28" aria-labelledby="pillars-heading">
      ...
      <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-px bg-[var(--border-default)] border border-[var(--border-default)] stagger-children">
        {capabilities.map((item) => <div key={item.href} className={`group flex min-h-[260px] sm:min-h-[300px] flex-col p-5 sm:p-7 md:p-9 bg-[var(--bg-deep)] border-l-2 ${item.line} hover:bg-[var(--bg-surface)] transition-colors`}>
    ...
    <section id="offerings" className="border-y border-[var(--border-default)] bg-[var(--bg-surface)]" aria-labelledby="offerings-heading"><div className="page-container py-20 md:py-28"><div className="text-center"><div className="eyebrow text-[var(--accent-orange)]">Flagship Offerings</div><h2 id="offerings-heading" className="section-title mx-auto mt-3 max-w-3xl text-center text-4xl md:text-6xl lg:text-7xl">Precision-built systems for serious builders.</h2></div><div className="mt-14 divide-y divide-[var(--border-default)] border-y border-[var(--border-default)]">{[['Personal AI Mastery','A practical program for building and running your own personal AI systems with extreme operational security and long-term autonomy.','/forge/','01','View Curriculum','text-[var(--accent-purple)]'],['Growth Boost','Strategic Web3 growth — community building, public good initiatives, and fundraising with OpSec foundations baked in.','/web3/','02','Explore Web3','text-[var(--accent-orange)]'],['Tailored Multi-Agent Systems','Custom single and multi-agent architectures built around your workflows, data, and use case — shipped and hardened.','/ai/','03','Explore AI Engineering','text-[var(--accent-cyan)]']].map(([title, text, href, index, cta, ctaColor]) => <Link key={title} href={href} className="group grid md:grid-cols-[5rem_1fr_auto] gap-5 items-start py-7 hover:px-3 transition-all"><span className="font-mono text-xs text-[var(--accent-cyan)]">{index}</span><span><h3 className="text-xl font-semibold">{title}</h3><p className="mt-2 max-w-xl text-sm text-[var(--text-secondary)]">{text}</p></span><span className={`text-sm ${ctaColor}`}>{cta} <Arrow /></span></Link>)}</div></div></section>

    <section className="border-y border-[var(--border-default)] bg-[var(--bg-surface)] pb-24" aria-labelledby="cta-heading"><div className="page-container pt-8 md:pt-14"><div className="relative w-full overflow-hidden"><div className="relative z-10"><div className="relative overflow-hidden border border-[var(--border-default)] bg-[var(--bg-deep)]"><div className="relative h-56 md:h-64 overflow-hidden"><div className="absolute left-8 top-6 z-10 md:left-10"><div className="eyebrow text-[var(--accent-cyan)]">Stay in the Loop</div></div><div className="absolute inset-0"><InkGarden compact /></div><div className="absolute inset-0 bg-gradient-to-r from-[var(--bg-deep)] via-[var(--bg-deep)]/70 to-transparent" /><h2 id="cta-heading" className="section-title absolute inset-x-0 top-1/2 max-w-xl -translate-y-1/2 p-8 md:p-10">High-signal updates only.</h2><p className="absolute bottom-0 right-0 left-auto z-10 max-w-lg p-8 pb-4 md:p-10 md:pb-6 text-[var(--text-secondary)]">Curated intel from research labs, key industry leaders, emerging talents, and engineering communities.</p></div></div><div className="mt-10"><CuratedIntel /></div><div className="mt-6 flex flex-wrap items-center justify-between gap-4"><div className="flex flex-wrap gap-3"><Link href="/tutorials/" className="button-secondary">Tutorials <Arrow /></Link><Link href="/blog/" className="button-secondary">Blog <Arrow /></Link></div><Link href="/contact/" className="button-secondary">Get in touch <Arrow /></Link></div></div></div></div></section>
    </div>
  </main>;
}
