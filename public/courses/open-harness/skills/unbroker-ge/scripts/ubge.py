#!/usr/bin/env python3
"""unbroker-ge engine — consent, queue, ledger. stdlib only."""
...
def data_dir() -> Path:
    raw = os.environ.get("UBGE_DATA_DIR")
    if raw:
        return Path(raw)
    hermes = os.environ.get("HERMES_HOME")
    if hermes:
        return Path(hermes) / "unbroker-ge"
    return Path.home() / ".hermes" / "unbroker-ge"
...
def cmd_intake(args: argparse.Namespace) -> None:
    ...
    dossier = {
        ...
        "canton": (args.canton or "GE").strip().upper(),
        "consent": True,
        "consent_at": utc_now(),
...
def render_letter(dossier: dict[str, Any], target_id: str, listing_urls: list[str]) -> str:
    ...
    template = (SKILL_DIR / "templates" / "opposition.fr.txt").read_text(encoding="utf-8")
    urls = "\n  ".join(listing_urls) if listing_urls else "(aucune fiche confirmée — opposition préventive)"
    return (
        ...
        json.dumps(
            {
                ...
                "to": spec.get("email"),
                "path": str(out),
                "subject": f"Opposition au traitement — nLPD — {spec['name']}",
                "body": body,
            },
            ensure_ascii=False,
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    inn.add_argument("--canton", default="GE")
